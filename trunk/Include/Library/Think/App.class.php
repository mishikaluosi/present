...
class App {
    ...
    static public function init() {
		...
		//选择判断是否开启手机端
		$CFG_MOBILE_AUTO = M("config") ->where("name = 'CFG_MOBILE_AUTO'") ->find();
		$CFG_MOBILE_AUTO = $CFG_MOBILE_AUTO["value"];

		//核心项目启动判断选择设备端口
		//获取当前的域名
		$url = 'http://'.$_SERVER['SERVER_NAME'].':'.$_SERVER["SERVER_PORT"].$_SERVER["REQUEST_URI"];
		if( BIND_VIEW == "Mobile" &&!strstr($url,BIND_VIEW))
		{
			// if($CFG_MOBILE_AUTO == "1")
			// {
			// 	$param = array();
