...
/**
 * 编译获取SQLITE数据库中文全连接
 * @param string $apppath sqlite文件名
 * @return path
 */
function GetSqlitePath($filename)
{
	$path = iconv("GB2312","UTF-8",dirname(__FILE__));
	//切割路径字符串
	$arr = explode('Common',$path);
	//重组路径
	$path = $arr[0].$filename;
	return $path;
}
...
function get_url($cate, $id = 0, $jumpflag = false, $jumpurl = '') {
    ...
    $links = $cate['links'];
    if ($cate['type'] == 1) {
        $firstChar = substr($links, 0, 1);
        if ($firstChar == '@') {//内部
            //不存在文档id,也无路由情况            
            $links = substr($links, 1);//
            $firstChar = substr($links, 0, 1); 
            //$url = ($firstChar != '/') ? U($module.$links,'') : U(''.$links,'');   
			$host = $_SERVER['HTTP_HOST'];
			$localurl = $_SERVER['PHP_SELF'];
			if(strstr($localurl,"/index.php"))
			{
				$arr = explode("/index.php",$localurl);
				$localurl = $arr[0];
			}
			$index = strlen($localurl)-1;
			if(substr($localurl,$index,1) != "/" && substr($url,0,1) != "/")
				$localurl = $localurl.'/';
			$url = 'http://'.$host.$localurl.$links ;
        ...
        if(C('URL_ROUTER_ON') == true) {
            $url = $id > 0 ? U($module.''.$links.'/'.$id,'') : U('/'.$links,'', '');
        }else {
...
function get_position($typeid = 0, $sname = '', $surl = '', $ismobile = false, $delimiter = '&gt;&gt;') {
	...
	$position = '<a href="'. U('Index/index') .'">首页</a>';
...
function get_item($group = 'animal', $update  = 0) {
    ...
    if ($update  || !$itme_arr) {
        $itme_arr = array();
        $temp = M('iteminfo')->where(array('itemgroup' => $group))->order('sort,id')->select(); 
        ...
        if (!empty($mat[1])) {
            ...
            foreach ($mat[1] as $k => $v) {          
                ...
                foreach ($detail as $k2 => $v2) {                
                    ...
                    $replace = array($v2['id'], $v2['title'], $upload.$v2['content'],$v2['url'], $v2['sort'],
...
function get_nowthumb()
{
	//获取当前的链接地址
	$web_url =  $_SERVER["HTTP_REFERER"];
	//如果包含产品链接，则添加项目为产品
	if(strstr($web_url,'/Manage/Product/'))
	{
		$imgtbFixWidth = explode(',', C('CFG_PRODUCT_THUMB'));
		$lit_show_type = C('CFG_PRODUCT_TYPE');
	}
	//如果包含案例链接，则添加项目为案例
	elseif(strstr($web_url,'/Manage/Cases/'))
	{
		$imgtbFixWidth = explode(',', C('CFG_CASE_THUMB'));		
		$lit_show_type = C('CFG_CASE_TYPE');
	}
	//如果包含文章链接，则添加项目为文章
	elseif(strstr($web_url,'/Manage/Article/'))
	{
		$imgtbFixWidth = explode(',', C('CFG_NEWS_THUMB'));		
		$lit_show_type = C('CFG_NEWS_TYPE');
	}
	else
	{
	...
	return array('imgtbFixWidth'=>$imgtbFixWidth,'lit_show_type'=>$lit_show_type);
}
 ...
 * @return string
 */
function get_picture($pic, $width = 0, $height = 0) {
	if(!empty($pic))
	{
		$pic = explode('.',$pic);
		if(strstr($pic[0],'!'))
		{
			$pic_str = explode('!',$pic[0]);
			$str = $pic_str[0];
		}
		else
			$str = $pic[0];
		...
		$litpic = $str."!".$width."X".$height.".".$pic[1];
		
		//判断改缩略图是否存在
		if(file_exists("./uploads/".$litpic))	
			return $litpic;
		else
		{
			//判断原图是否存在
			$real_path = "./uploads/".$str.".".$pic[1];
            //return $real_path;
			if(file_exists($real_path))
			{
				//重新生成缩略图
				$think_img = new \Think\Image(); 
				$think_img->open($real_path)->thumb($width,$height,3)->save("./uploads/".$litpic);
				return $litpic;
			}
			else
 ...
 * @return string
 */
function go_link($weburl = 'https://www.baidu.com/', $rnd = 0, $flag = 1) {
...
function get_region_all($level=3){
    $return_arr=array();
...
function get_region_jsdata($type=null){
    $return_arr=array();
    $top=get_region_bypid();
    if($type=='select'){
        $add_index=1;
    ...
    foreach ($top as $k=>$t){
        ...
        foreach ($child as $kk=>$c) {
            ...
            $child_tmp[$kk]['name']=$c['region_name'];;
            $child_tmp[$kk]['areaList']=$end_tmp;
        }
...
function get_region_bypid($pid=1,$type=1){
    if(!is_numeric($pid)||!is_numeric($type)){
        return null;
    }
    $area= M('region')->where(array('parent_id'=>$pid,'region_type'=>$type))->order('sort asc,region_id asc')->select();
    return $area;
}
...
function get_region_name($id){
    ...
    $info=M('region')->where(array('region_id'=>$id))->find();
    if($info&&$info['region_name']){
        return $info['region_name'];
    }
    return '';
}
function get_zc_name($id){
    if(!is_numeric($id)){
        return '';
    }
    $info=M('zc')->where(array('id'=>$id))->find();
    if($info&&$info['name']){
        return $info['name'];
    }
    return '';
...
function get_zc_byname($name){
    if(empty($name)){
        return '';
    }
    $info=M('zc')->where(array('name'=>$name))->find();
    return $info;
}
