...
class Yang extends TagLib {
	...
	protected $tags = array(
		...
		//通用列表
		'list'	=> array(
			'attr'	=> 'flag,typeid,titlelen,infolen,orderby,keyword,limit,pagesize,pageroll,pagetheme,out',
			'close'	=> 1,
		),
		...
		//栏目
		'catlist'	=> array(
			'attr'	=> 'typeid,type,orderby,limit,flag,modelid,in',//flag为是否全部显示
			'close'	=> 1,
		),
	...
	public function _artlist($attr, $content) {
		...
		$texta = empty($attr['texta'])? '': trim($attr['texta']);
		$textb = empty($attr['textb'])? '': trim($attr['textb']);
		$textc = empty($attr['textc'])? '': trim($attr['textc']);
		$textd = empty($attr['textd'])? '': trim($attr['textd']);
		$texte = empty($attr['texte'])? '': trim($attr['texte']);
...
str;
	...
	public function _list($attr, $content) {
		...
		//$out = string2filter($out,',', true);
		
		$out = empty($attr['out'])? '': trim($attr['out']);
		$flag = flag2sum($flag);
...
<?php
	...
	if (!empty(\$_out)) {
		\$where[\$_tablename.'.id'] = array('neq', \$_out);
	}
...
str;
	...
	public function _catlist($attr, $content) {
		...
		$in = empty($attr['in']) ? "" : $attr['in'];//print_r($typeid);die;
		$str = <<<str
<?php
	...
	if (!empty(\$_in)) {
		\$__catlist = Common\Lib\Category::getLevelInId(\$__catlist, \$_in_arr);
	}
...
<?php
	...
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['tablename']) ) {
		...
		//上一条记录
        \$_vo= D2('ArcView', \$cate['tablename'])->where(array(\$cate['tablename'].'.status' => 0, 'cid' => \$content['cid'], 'publishtime' => array('gt',\$content['publishtime'])))->order('publishtime ASC')->find();
...
<?php
	if(empty(\$content['id']) || empty(\$content['cid']) || empty(\$cate['tablename']) ) {
		...
		//下一条记录
        \$_vo= D2('ArcView',\$cate['tablename'])->where(array(\$cate['tablename'].'.status' => 0, 'cid' => \$content['cid'], 'publishtime' => array('lt',\$content['publishtime'])))->order('publishtime DESC')->find();
...
str;
	...
	public function _wapsq($attr, $content) {
		return C('CFG_WAPSQ');
	}
