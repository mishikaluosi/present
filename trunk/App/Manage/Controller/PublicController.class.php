...
class PublicController extends CommonController {
	...
	public function getFileOfImg() {
        ...
        //读取缩略图配置信息
        $imgtbSize = explode(',', C('CFG_DEFAULT_THUMB'));//配置缩略图第一个参数
        ...
        if(empty($_FILES)){              
            ...
            if(isset($info) && is_array($info)){ 
                ...
                foreach ($info as $k => $v) {
                	...
                	//$v['url'] = 'uploads/'. $v['savepath']. $v['savename'];
                	...
                	if (!empty($imgTSize)) {
						//根据当前的链接获取适当参数
						$arr = get_nowthumb();
						$imgtbFixWidth = $arr["imgtbFixWidth"];
						//制作缩略图
						$v['turl'] = get_picture($v['url'],$imgtbFixWidth[0],$imgtbFixWidth[1]);
					}
	...
	**/	
	...
	public function _uploadPicture() {
		...
		if($info = $upload->upload()) {
			...
			$arr = get_nowthumb();
			$imgtbFixWidth = $arr["imgtbFixWidth"];
			$lit_show_type = $arr["lit_show_type"];
			...
			$imgtbFixArray[] = array('w' => intval($imgtbFixWidth[0]), 'h' => intval($imgtbFixWidth[1]));
			...
			if (!empty($imgtbFixArray) || !empty($imgtbArray)) {
				...
				foreach ($info as $k => $file) {
					...
					//根据.来切割连接字段，重组到为新的路径
					$lit_arr = explode('.', $file['savename']);
					
					//重新组成新的缩略图路径
					$lit_path = $upload->rootPath. $file['savepath'].$lit_arr[0];
					
					
					//优先生成原图的缩略图
					foreach ($imgtbFixArray as $v) {
						$strSuffix = '!'.$v['w'].'X'.$v['h'];
						$think_img->open($real_path)->thumb($v['w'],$v['h'],3)->save($lit_path.$strSuffix.'.'.$ext_dest);
					}
					//是否需要水印
					$Iswater = C("CFG_WATER_IF");					
					$web_url =  $_SERVER["HTTP_REFERER"];
					if( $Iswater == "1" && !strstr($web_url,'/Manage/System/'))
					{
						
						//确认水印形式 0,文字 1,图片
						$watertype = C("CFG_WATER_TYPE");
						
						//水印的位置
						$water_mark = C("CFG_WATER_MARK");
						
						//生成的水印的路径
						$water_path = $upload->rootPath.$file['savepath'].$pic_name.time();
						
						
						
						//确认水印种类 0,文字 1,图片 2，平铺图片
						if($watertype == "1")
						{
							$water_img = C("CFG_WATER_IMG");
							if(!empty($water_img) && file_exists("./uploads/".$water_img))
								//获取充当水印的图片
								$water_img = './uploads/'.C("CFG_WATER_IMG");
							else
								$water_img = './uploads/system/bestop.png';
							
							
							$think_img->open($real_path)->water($water_img,$water_mark)->save($real_path); 
						}
						else
						{
							//字体水印参数设置
							//水印的内容
							$water_text = C("CFG_WATER_TEXT");
							//水印的大小
							$water_size = C("CFG_WATER_SIZE");
							//水印的颜色
							$water_color = C("CFG_WATER_COLOR");						
							$think_img->open($real_path)->text($water_text,'./1.ttf',$water_size,$water_color,$water_mark)->save($real_path); 
						}
					}
	...
	//文件/夹管理
	function browseFile($spath = '', $stype = 'picture') {
		...
		for ($i=0; $i < count($list); $i++) { 
			...
			$list[$i]['ShowUrl'] = str_replace($UPLOAD,"",$url);
		}
		//p($list);
	...
	public function editor($editorId = 'content', $width = '800', $height = '600', $type = 'ueditor') {
