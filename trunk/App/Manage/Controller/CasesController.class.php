...
class CasesController extends CommonContentController {
	...
	public function index() {		
		...
		if ($pid) {
			...
			$where = array('Cases.status' => 0, 'cid' => array('in', $idarr));
		}else {
			$where = array('Cases.status' => 0);
		}

		if (!empty($keyword)) {
			$where['Cases.title'] = array('LIKE', "%{$keyword}%");
		}
		
		
		//import('Class.Page', APP_PATH.'Lib/');
		$count = D2('ArcView','Cases')->where($where)->count();
		$page = new \Common\Lib\Page($count, 10);		
		$page->rollPage = 7;
		$page->setConfig('theme','%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
		$limit = $page->firstRow. ',' .$page->listRows;
		$art = D2('ArcView','Cases')->nofield('content')->where($where)->order('Cases.id DESC')->limit($limit)->select();
		
		$this->assign('pid', $pid);
		$this->assign('keyword', $keyword);
		$this->assign('page', $page->show());
		$this->assign('vlist', $art);
		$this->assign('type', '案例列表');
	...
	public function addPost() {
		...
		if( C('CFG_CASE_USE') == "1" )
		{
			$imgtbSize = explode(',', C('CFG_CASE_THUMB'));//配置缩略图第一个参数
			$data['litpic'] = get_picture($data['litpic'],$imgtbSize[0],$imgtbSize[1]);
		}
		...
		if($id = M('Cases')->add($data)) {
			...
			//$this->success('添加案例成功',U('Cases/index', array('pid' => $pid)));
			$this->redirect('Cases/index', array('pid' => $pid));
		}else {
			$this->error('添加案例失败');
		}
	}
	...
	public function editPost() {
		...
		if (false !== M('Cases')->save($data)) {
			...
			//$this->success('修改成功', U('Cases/index', array('pid' => $pid)));
			$this->redirect('Cases/index', array('pid' => $pid));
		}else {
	...
	public function move() {
		...
		if (IS_POST) {
			...
			if (false !== M($actionName)->where(array('id'=> array('in', $id)))->setField('cid',$cid)) {
				//$this->success('移动成功', U('Cases/index', array('pid' => $pid)));
				$this->redirect('Cases/index', array('pid' => $pid));
			}else {
				$this->error('移动失败');
			}			
	...
	public function trach() {
		...
		$where = array('Cases.status' => 1,'Cases.cid' => I('pid'));
		$count = D2('ArcView','Cases')->where($where)->count();
		...
		$page->setConfig('theme','%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
		$limit = $page->firstRow. ',' .$page->listRows;
		$art = D2('ArcView','Cases')->nofield('content')->where($where)->limit($limit)->select();
	...
	public function del() {
		...
		if (false !== M('Cases')->where(array('id' => $id))->setField('status', 1)) {

			del_cache_html('Show/index_*_'. $id.'.', false, 'show:index');				
			//$this->success('删除成功', U('Cases/index', array('pid' => $pid)));
			$this->redirect('Cases/index', array('pid' => $pid));
	...
	public function delBatch() {
		...
		if (false !== M('Cases')->where(array('id' => array('in', $idArr)))->setField('status', 1)) {
			...
			//. M('case')->getlastsql();
			//$this->success('批量删除成功', U('Cases/index', array('pid' => $pid)));
			$this->redirect('Cases/index', array('pid' => $pid));
	...
	public function restore() {
		...
		if (false !== M('Cases')->where(array('id' => $id))->setField('status', 0)) {
			
			//$this->success('还原成功', U('Cases/trach', array('pid' => $pid)));
			$this->redirect('Cases/trach', array('pid' => $pid));
	...
	public function restoreBatch() {
		...
		if (false !== M('Cases')->where(array('id' => array('in', $idArr)))->setField('status', 0)) {
	...
	public function clear() {
		...
		$pid = I('get.pid', 0, 'intval');
		$modelid = D2('ArcView','Cases')->where(array('id' => $id))->getField('modelid');

		if (M('Cases')->delete($id)) {
			// delete picture index
			if ($modelid) {
				M('attachmentindex')->where(array('arcid' => $id , 'modelid' => $modelid ))->delete();//test
			}
			//$this->success('彻底删除成功', U('Cases/trach', array('pid' => $pid)));
			$this->redirect('Cases/trach', array('pid' => $pid));
		}else {
			$this->error('彻底删除失败');
		}
	...
	public function clearBatch() {
		...
		$modelid = D2('ArcView','Cases')->where(array('id' => $idArr[0]))->getField('modelid');//

		if (M('Cases')->where($where)->delete()) {
			// delete picture index
			if ($modelid) {
	...
	public function csort(){
	    $csort = I('post.csort','', 'intval');
	    $id = I('post.id', '', 'intval');
	    $data['csort'] = $csort;
	    $csorts=M('cases')->where("id=$id")->data($data)->save();
	    echo "$csorts";
	}
