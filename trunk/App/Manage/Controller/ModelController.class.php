...
class ModelController extends CommonController {
	...
	public function addPost() {
		...
		if (!$result){
            ...
            if ($model->add($data)) {
				...
				//更新导航数据数据
				$this->UpdateCategoryTemplate($id,$data["template_list"],$data["template_show"]);
	...
	public function UpdateCategoryTemplate($id,$template_list,$template_show)
	{
		$data["template_list"] = $template_list;
		$data["template_show"] = $template_show;
		M("category") -> where(" modelid = ".$id) -> save($data);
	}
	...
	public function del() {
		...
		if (M('model')->delete($id)) {
			...
			$this->redirect('Model/index', array('id' => $id));
		}else {
			$this->error('删除失败');
		}
	}
