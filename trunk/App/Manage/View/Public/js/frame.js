	...
	/*修改-刘纯纯 2015-11-04*/
		...
		function sidebar(){
			...
			$('#menu1 dd').each(function(){
				$(this).children("ul").children("li:eq(0)").addClass("aa"); //最上面的大分类
				})
			$('#menu1 dd ul li').each(function() { 
			      $(this).next(".ulul").prev("li").addClass("bb"); //如果下面一个class 是 ulul 。则当前增加样式bb   
				 // $(this).children("a").removeAttr("href");
            });
			$('#menu dd ul li a').click(function(){
				$('#menu1 dd ul ul').removeClass("bbbb");//点击左边大分类，所有的二级分类初始化
			})		
			
			$('#menu1 dd ul li.bb').click(function(){			
				$(this).next(".ulul").addClass("bbbb");
				$(this).children("a").addClass("thisclass");
				$(this).siblings("li").next(".ulul").removeClass("bbbb");
				$(this).siblings("li").children("a").removeClass("thisclass");
			})
