...
//ajax获取信息
function app_getinfo_data(url,data, func) {
    //var app_info = '&city=' + app_pageval('city') + '&location=' + app_getval('location');
...
function app_page(id, func) {
	...
	var height = $("#"+id).height() > window_h ? window_h : $("#"+id).height();
	//height = height < window_h/2 ? window_h/2 : height;
	$("#"+id).css({"position":"fixed", "width":"100%", "height":height, "z-index":100, "bottom": 0, "left":0, "background-color":"#fff", "overflow-y":"auto"}).fadeIn(400).addClass("app_pagemain");
