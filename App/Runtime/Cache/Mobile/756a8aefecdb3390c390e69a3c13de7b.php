...
<div class="content">
    <div class="subnav1" style="margin-top:24px;">

        <?php if(is_array($clist)): foreach($clist as $key=>$vo): ?><a href="<?php echo U('article/index',array('cid'=>$vo['id']));?>" title="<?php echo ($vo["name"]); ?>" <?php if($cid == $vo['id']): ?>class="sel"<?php endif; ?>><?php echo ($vo["name"]); ?></a><?php endforeach; endif; ?>
        <div class="clear"></div>
    </div>

    <div class="fenlei_list zx_list">
        <ul>
            <?php if(is_array($vlist)): foreach($vlist as $key=>$vo): ?><li>
                <a href="<?php echo U('article/detail',array('id'=>$vo['id']));?>" title="<?php echo ($vo["title"]); ?>">
                    <?php echo ($vo["title"]); ?>
                    <p class="cbbb font12 num"><?php echo (date("Y-m-d",$vo["publishtime"])); ?></p>
                </a>
                <i></i>
