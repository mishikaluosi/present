...
<input type="hidden" id="index_url" value="<?php echo U('User/eventTongji',array('type'=>$type));?>">
...
<div class="main">
    <div class="operate" style="height: 40px;line-height: 40px;">
        <?php if($type == area): ?><select style="width: 24%;" id="zc_search" name='zc_search'>
        ...
        <select style="width: 24%;" id="event_search" name='event_search'>
            ...
            <?php if(is_array($event)): foreach($event as $key=>$value): ?><option <?php if($e_name==$value){ ?>selected="selected"<?php } ?> value="<?php echo ($value); ?>"><?php echo ($value); ?></option><?php endforeach; endif; ?>
        </select>
        <input style="width: 24%;" type='text' id="start_date" class="start_date" value="<?php echo ($start_date); ?>" placeholder="开始时间" readonly/>
        <input style="width: 24%;" type='text' id="end_date" class="end_date" value="<?php echo ($end_date); ?>" placeholder="结束时间" readonly/>
    </div>
    <div class="operate" style="text-align: center;font-size: 1em;height: 40px;line-height: 40px;">
        <span style="margin-left: 10px;">活动数：<?php echo ($event_total['e_num']); ?></span>
        <span style="margin-left: 10px;">预约人数：<?php echo ($event_total['app_num']); ?></span>
        <span style="margin-left: 10px;">签到人数：<?php echo ($event_total['check_num']); ?></span>
    </div>
    <div class="operate" style="text-align: center;font-size: 1em;height: 40px;line-height: 40px;">
        <span style="margin-left: 10px;">预约保费：<?php echo ($event_total['appointment_money']); ?></span>
        <span style="margin-left: 10px;">实际保费：<?php echo ($event_total['appointment_money_actual']); ?></span>
    </div>
    <table style="width:100%;font-size: 0.8em;line-height: 35px;" border="2" cellpadding=“10” cellspacing="0">
        ...
        <?php if(is_array($vlist)): foreach($vlist as $key=>$v): ?><tr>
                <?php if($type == area): ?><td><?php echo ($v["zc_name"]); ?></td>
                    <?php elseif($type == city): ?>
                    <td><?php echo ($v["areas"]); ?></td>
                    <?php else: ?>
                    <td><?php echo ($v["zc_name"]); ?></td><?php endif; ?>
