...
<script type="text/javascript" src="/App/Manage/View/Public/js/common.js"></script>
 <script language="JavaScript">
        <!--
        var URL = '/index.php?s=/Manage/Area';
        var APP	 = '/index.php?s=';
        var SELF='/index.php?s=/Manage/Area/index';
...
<div class="main">
    ...
    <div class="operate">
        <div class="left"><input type="button" onclick="goUrl('<?php echo U('Area/add', array('pid' => $pid));?>')" class="btn_blue btn_red" value="添加地区">
        <input type="button" onclick="doConfirmBatch('<?php echo U('Area/del', array('batchFlag' => 1,'pid' => $pid));?>', '确实要删除选择项吗？')" class="btn_blue btn_red" value="删除">
        <input type="button" onclick="doGoSubmit('<?php echo U('Area/sort', array('pid' => $pid));?>','form_do')" class="btn_blue btn_red" value="更新排序"></div>
        <?php if($pid > 0): ?><input type="button" onclick="goUrl('<?php echo U('Area/index', array('pid' => 0));?>')" class="btn_blue btn_green" value="返回顶级">
        <?php else: ?>
        <input type="button" onclick="goUrl('<?php echo U('Area/createJsArea', array('pid' => 0));?>')" class="btn_blue btn_red" value="生成JS"><?php endif; ?>
        </div>
        <div class="clear"></div>
    </div>
    <div class="list">    
