...
/**
 * 根据config的name获取对应name的数据
*/

function get_config($name){
	$config = M('config')->where(array('name'=>$name))->find();
	$value = $config['value'];
	return $value;
}
...
{
	$res = M("block") -> where(array('id'=>$id)) -> find();
	switch($res["blocktype"])
	{
		case "1":
			return strlen($res["content"]) > 10?mb_substr($res["content"],0,10,'utf-8').'...':$res["content"];
			break;
		case "2":
			return '<img src='.$res["content"].'  style="width:200px;height:100px">';
			break;
		case "3":
			return "富文本";
			break;
...
function GetTrach($cid,$tablename)
{
	$res = M($tablename) -> where(array('cid'=>$cid,'status'=>"1")) -> count();
	return $res;
}
