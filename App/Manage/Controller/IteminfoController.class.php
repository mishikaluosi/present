...
class IteminfoController extends CommonController {
	...
	public function index() {
		...
		$count = M('iteminfo')->where(array('itemgroup' => $group))->count();

		$page = new \Common\Lib\Page($count, 10);
		$page->rollPage = 7;
		$page->setConfig('theme','%HEADER% %FIRST% %UP_PAGE% %LINK_PAGE% %DOWN_PAGE% %END%');
		$limit = $page->firstRow. ',' .$page->listRows;
		$list = M('iteminfo')->where(array('itemgroup' => $group))->order('sort')->limit($limit)->select();
	...
	public function add() {
		...
		if (IS_POST) {
			...
			$vo = M('iteminfo')->where(array('itemgroup' => $data['itemgroup'], 'value' => $data['value']))->find();
			if ($vo) {
		...
		$data = M('iteminfo')->where(array('itemgroup' => $group))->field('MAX(value) as maxv')->find();
	...
	public function edit() {
		...
		if (IS_POST) {
			...
			$vo = M('iteminfo')->where(array('id' => array('neq', $data['id']), 'itemgroup' => $data['itemgroup'], 'value' => $data['value']))->find();
			if ($vo) {
				$this->error('枚举值已经存在，请重新填写');
			}
