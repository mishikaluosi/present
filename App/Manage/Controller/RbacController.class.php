...
class RbacController extends CommonController {
	...
	public function addUserPost() {
		...
		$zc_id=I('zc','');
        if(is_numeric($zc_id)){
            $tmp_zc=M('zc')->where(array('id'=>$zc_id))->find();
            if($tmp_zc){
                $userData['prov']=$tmp_zc['prov'];
                $userData['city']=$tmp_zc['city'];
                $userData['area']=$tmp_zc['area'];
                $userData['zc_id']=$zc_id;
            }
        }
	...
	public function editUser() {
		...
		if(is_numeric($zc_id)){
            $tmp_zc=M('zc')->where(array('id'=>$zc_id))->find();
            if($tmp_zc){
                $data['prov']=$tmp_zc['prov'];
                $data['city']=$tmp_zc['city'];
                $data['area']=$tmp_zc['area'];
                $data['zc_id']=$zc_id;
            }
        }
	...
	public function batch_user(){
	    $return_arr=array('cnt'=>0, 'ok'=>0, 'err'=>0);

	    $where='1=1';
        $return_arr['cnt'] = M('zc')->where($where)->count();
        $list = M('zc')->where($where)->order('sort ASC, id DESC')->select();

        $role_id = I('role_id',null);
        if(empty($role_id)){
            $this->error('错误的请求');exit();
        }

        foreach ($list as $k=>$v){
            $flag=false;
            if(!empty($v['tel'])){
                $chk=M('admin')->where(array('username'=>$v['tel']))->find();
            }else{
                $chk=false;
            }

            if($chk==false){
                $passwordinfo =get_password(substr( $v['tel'],-6));
                $tmp_data=array(
                    'username' => $v['tel'],
                    'realname' => $v['contact'],
                    'password' => $passwordinfo['password'],
        ...
        $this->success("职场共有{$return_arr['cnt']}条数据，本次成功同步{$return_arr['ok']}条", U('Rbac/index'));
        exit();
    }
