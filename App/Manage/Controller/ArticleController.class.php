...
class ArticleController extends CommonContentController {
	...
	public function addPost() {
		...
		$data['asort'] = I('asort','1','intval');
		
		$data["texta"] = I('texta', '', 'htmlspecialchars,rtrim');
		$data["textb"] = I('textb', '', 'htmlspecialchars,rtrim');
		$data["textc"] = I('textc', '', 'htmlspecialchars,rtrim');
		$data["textd"] = I('textd', '', 'htmlspecialchars,rtrim');
		$data["texte"] = I('texte', '', 'htmlspecialchars,rtrim');
		...
		if( C('CFG_NEWS_USE') == "1" )
		{
			$imgtbSize = explode(',', C('CFG_NEWS_THUMB'));//配置缩略图第一个参数
		...
		if($id = M('article')->add($data)) {
			...
			$this->redirect('Article/index',array('pid' => $pid));
		}else {
			$this->error('添加文章失败');
		}
	}
	...
	public function editPost() {
		...
		//缩略图赋值
		$imgtbSize = explode(',', C('CFG_NEWS_THUMB'));//配置缩略图第一个参数
		$pic = get_picture($data['litpic'],$imgtbSize[0],$imgtbSize[1]);
	...
	public function trach() {
		...
		$where = array('article.status' => 1,'article.cid' => I('pid'));
		$count = D2('ArcView','article')->where($where)->count();
	...
	public function restore() {
		...
		if (false !== M('article')->where(array('id' => $id))->setField('status', 0)) {
			...
			$this->redirect('Article/trach', array('pid' => $pid));
	...
	public function asort(){
	    $asort = I('post.asort', '', 'intval');
	    $id = I('post.id', '', 'intval');
	    $data['asort'] = $asort;
	    $asorts=M('Article')->where("id=$id")->data($data)->save();
	    echo "$asorts";
	}
