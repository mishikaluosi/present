...
class SystemController extends CommonController {
	...
	public function site2()	{
		if (IS_POST) {
			...
			if($ret !== false) {
				F('config/site', null);
				$this->success('修改成功',U('System/site2'));
		...
		$this->assign('vlist', $glist);
		$this->assign('configgroup', $configgroup);
		$this->assign('groupnum', count($configgroup)+1);
		$this->assign('configtype', get_item('configtype'));
		$this->display();
	...
	public function site3()	{
		if(IS_POST) {
			...
			if($ret !== false) {
				F('config/site', null);
				$this->success('修改成功',U('System/site3'));
		...
		/*$config   =   M("config")->order('sort')->select();
	...
	{
		$cache = I('cache');
		$data['HOME_HTML_CACHE_ON'] = $cache;
		$data['MOBILE_HTML_CACHE_ON'] = $cache;
		foreach ($data as $k => $v) {
