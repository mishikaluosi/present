...
class ItemgroupController extends CommonController {
	...
	public function del() {
		...
		$child= $Model->table(C('DB_PREFIX'). 'iteminfo I')->join('inner join '. C('DB_PREFIX').'itemgroup G on I.itemgroup = G.name')->where(array('G.id' => $id))->getField('I.id');
		if($child) {
	...
	public function delBatch() {
		...
		//getField('id'),返回一个结果，getField('id',true),返回满足的所有(数组)
		$Model =M();
		$child= $Model->table(C('DB_PREFIX'). 'iteminfo I')->join('inner join '. C('DB_PREFIX').'itemgroup G on I.itemgroup = G.name')->where(array('G.id' => array('in', $idArr)))->getField('I.id');
		if($child) {
			$this->error('请先删除分组下的联动信息，再删除分组');
		}
